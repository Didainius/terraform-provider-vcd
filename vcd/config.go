...
type Config struct {
	...
	MaxRetryTimeout int
	InsecureFlag    bool
	// UseAdfsSaml flags that SAML is to be used as authentication. It will try to lookup SAML
	// endpoint endpoint and use `/adfs/services/trust/13/usernamemixed` on the ADFS iDP
	// Username and password should be specified in friendly format then `test@test-domain.net`
	UseAdfsSaml bool
}
...
func (c *Config) Client() (*VCDClient, error) {
	...
	if c.UseAdfsSaml {
		log.Printf("[INFO]: Using SAML ADFS endpoint '/adfs/services/trust/13/usernamemixed' with user: %s", c.User)
		err = ProviderAuthenticateSaml(vcdClient.VCDClient, c.User, c.Password, c.SysOrg)
	} else {
		log.Printf("[INFO]: Using local user login with user: %s", c.User)
